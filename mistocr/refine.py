...
# %% auto #0
__all__ = ['logger', 'prompt_fix_hdgs', 'prompt_fix_hdgs_suffix', 'describe_img_prompt', 'get_hdgs', 'add_pg_hdgs', 'read_pgs_pg',
           'fmt_hdgs_idx', 'HeadingCorrection', 'HeadingCorrections', 'fix_hdg_hierarchy', 'mk_fixes_lut',
           'apply_hdg_fixes', 'fix_hdgs', 'ImgDescription', 'describe_img', 'limit', 'parse_r', 'describe_imgs',
           'save_img_descs', 'add_descs_to_pg', 'add_descs_to_pgs', 'add_img_descs']

# %% ../nbs/01_refine.ipynb #fcf0aa93
...
# %% ../nbs/01_refine.ipynb #bd29f0ab
logger = logging.getLogger(__name__)
logging.basicConfig(level=logging.WARNING, format='%(name)s - %(levelname)s - %(message)s')
logger.setLevel(logging.INFO)

# %% ../nbs/01_refine.ipynb #8d47c62c
def get_hdgs(
...
def fix_hdg_hierarchy(
    ...
    prompt = (prompt + prompt_fix_hdgs_suffix).format(headings_list=fmt_hdgs_idx(hdgs))
    kw = dict(model=model, messages=[{"role": "user", "content": prompt}], response_format=HeadingCorrections, api_key=api_key)
    if reasoning_effort: kw['reasoning_effort'] = reasoning_effort
    r = completion(**kw)
    fixes = json.loads(r.choices[0].message.content)['corrections']
    return {o['index']: o['corrected'] for o in fixes}


# %% ../nbs/01_refine.ipynb #b7cb8190
